...
include_once(ROOT_PATH . '/app/helpers/validateSlider.php');
include_once(ROOT_PATH . '/app/helpers/middleware.php');

$table = 'sliders';

$sliders = selectAll($table);
...
if (isset($_GET['id'])) {
    $slider = selectOne($table, ['id' => $_GET['id']]);
    $id = $slider['id'];
    $title = $slider['title'];
    $link = $slider['link'];
    $body = $slider['body'];
...
if (isset($_POST['add-slider'])) {
    ...
    if (count($errors) === 0) {
        unset($_POST['add-slider']);
        $_POST['published'] = isset($_POST['published']) ? 1 : 0;

        //// To avoid cross-site scripting and remove html from body of sliders
        $_POST['body'] = htmlentities($_POST['body']);

        $slider_id = create($table, $_POST);
...
if (isset($_POST['update-slider'])) {
    ...
    if (!empty($_FILES['image']['name'])) {
        $image_name = time() . '-' . $_FILES['image']['name'];
        $destination = ROOT_PATH . '/img/slider_img/' . $image_name;
    ...
    if (count($errors) === 0) {
        $id = $_POST['id'];
        unset($_POST['update-slider'], $_POST['id']);
        $_POST['published'] = isset($_POST['published']) ? 1 : 0;

        //// To avoid cross-site scripting and remove html from body of sliders
        $_POST['body'] = htmlentities($_POST['body']);

        $slider_id = update($table, $id, $_POST);
